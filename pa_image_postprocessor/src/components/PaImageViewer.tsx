...
import { DEFAULT_PD_ZERO_ADC_CODE } from "../config";
import {
...
function valuesInTimeWindow(trace: PaFrameTrace, startNs: number, endNs: number, processing: PaImageProcessing): number[] {
  const intervalNs = Number.isFinite(processing.sampleIntervalNs) && processing.sampleIntervalNs > 0
    ? processing.sampleIntervalNs
    : 1;
  const sampleStart = Math.max(0, Math.floor(processing.sampleStartIndex));
  const sampleEnd = Math.max(sampleStart, trace.current_ua.length - Math.max(0, Math.floor(processing.sampleEndTrim)));
  const traceLength = sampleEnd - sampleStart;
  const windowStart = Math.min(startNs, endNs);
  const windowEnd = Math.max(startNs, endNs);
  const startOffset = Math.ceil(windowStart / intervalNs);
  const endOffset = Math.min(traceLength, Math.ceil(windowEnd / intervalNs));
  if (startOffset < 0 || startOffset >= endOffset || startOffset >= traceLength) return [];

  return trace.current_ua
    .slice(sampleStart + startOffset, sampleStart + endOffset)
...
export default function PaImageViewer({
  ...
  const saveRoiDefaults = () =>
    runBusy("Save ROI Defaults", async () => {
      ...
      setMessage(
        `Saved ROI ${processing.ptpStartNs}-${processing.ptpEndNs} ns and baseline ${processing.baselineStartNs}-${processing.baselineEndNs} ns as local defaults.`,
      );
    });
  ...
  return (
    <section className="panel pa-image-viewer" aria-label="PA Image Viewer">
      <div className="pa-image-header">
        ...
        {onBack ? <div className="pa-image-header-actions">
