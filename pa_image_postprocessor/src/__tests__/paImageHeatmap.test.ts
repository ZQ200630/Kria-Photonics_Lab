...
describe("PA image heatmap layout", () => {
  ...
  it("maps displayed pixels back through a 90-degree image rotation", () => {
    const layout = resolvePaImageHeatmapLayout({
      cssWidth: 520,
      cssHeight: 360,
      width: 4,
    ...
    expect(
      resolvePaImagePixelFromCanvasPoint({
        canvasX: layout.x0 + layout.gridWidth * 0.25,
        canvasY: layout.y0 + layout.gridHeight * 0.125,
        layout,
        rotation: 90,
      }),
    ).toEqual({ x: 0, y: 0 });
  ...
  it("rotates selected-pixel overlays with the image raster", () => {
    const layout = resolvePaImageHeatmapLayout({
      cssWidth: 520,
      cssHeight: 360,
      width: 4,
    ...
    const rectangle = resolvePaImageSelectedPixelRectangle({ selectedPixel: { x: 0, y: 0 }, layout, rotation: 90 });

    expect(rectangle).not.toBeNull();
    expect(rectangle?.left).toBeCloseTo(layout.x0);
    expect(rectangle?.top).toBeCloseTo(layout.y0);
    expect(rectangle?.width).toBeCloseTo(layout.gridWidth / 2);
    expect(rectangle?.height).toBeCloseTo(layout.gridHeight / 4);
  });
