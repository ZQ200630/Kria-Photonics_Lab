...
describe("PA Image Viewer layout", () => {
  ...
  it("aligns trace quality windows with the sample-start offset used by image reconstruction", () => {
    const currentUa = Array.from({ length: 20 }, (_value, index) => index);
    const metrics = paTraceQualityMetrics(
      {
        path: "trace.bin",
        frame_index: 0,
        ...
        metadata: null,
        samples: [],
        time_ns: currentUa.map((_value, index) => index * 8),
    ...
    expect(metrics).not.toBeNull();
    expect(metrics?.baselineSampleCount).toBe(2);
    expect(metrics?.signalSampleCount).toBe(2);
    expect(metrics?.baselineMeanUa).toBe(10.5);
    expect(metrics?.signalPeakToPeakUa).toBe(1);
  });
